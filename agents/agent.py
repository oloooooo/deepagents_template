...
- :class:`AgentMemory` 只管记忆读写（``aget_state`` / ``alist_memories`` / 写入四件套），
  借用运行中 agent 的 graph 与 store，自身无生命周期，由 ``agent.memory`` 暴露。
...
class GeneralAgent:
    ...
    async def __aenter__(self) -> "GeneralAgent":
        ...
        async with AsyncExitStack() as stack:
            ...
            self._graph = graph
            self.memory = AgentMemory(graph, store)
        logger.info(
...
class AgentMemory:
    """记忆读写：借用运行中 :class:`GeneralAgent` 的 graph 与 store，自身无生命周期。
    ...
    def __init__(self, graph: Any, store: AsyncPostgresStore) -> None:
        self._graph = graph
        self._store = store
